...
export default function DailyDesk({ tasks = [] }: { tasks: any[] }) {
  ...
  return <section className="space-y-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2"><div className="rounded-xl border bg-card p-4 space-y-2"><h2 className="font-semibold">Bugünün duruşmaları</h2>{hearings.isPending ? <p>Yükleniyor…</p> : hearings.isError ? <button onClick={() => hearings.refetch()}>Duruşmalar alınamadı. Tekrar dene</button> : todays.length ? todays.map((h: any) => <Link className="block rounded-lg bg-muted/40 p-3 text-sm" key={h.id} to={caseRecordHref(h.caseId, 'hearing', h.id)}>{formatDateTime(h.hearingDate)} · {h.caseTitle}<span className="block text-muted-foreground">{h.courtName}</span></Link>) : <p className="text-sm text-muted-foreground">Bugün bekleyen duruşma yok.</p>}</div>
    <div className="rounded-xl border bg-card p-4 space-y-2"><h2 className="font-semibold">Bugün ve geciken işler</h2>{dueTasks.length ? dueTasks.map(task => <Link key={task.id} className="block rounded-lg bg-muted/40 p-3 text-sm" to={taskHref(task)}>{task.title}<span className="block text-muted-foreground">{formatDateTime(task.dueDate)}</span></Link>) : <p className="text-sm text-muted-foreground">Özette bugün için görev görünmüyor.</p>}<Link className="block text-sm text-law-accent" to="/cases?tracking=check">Kontrol edilecek dosyalar →</Link><Link className="block text-sm text-law-accent" to="/tasks">Tüm görevler →</Link></div></div>
